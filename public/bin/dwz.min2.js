...
if(json.statusCode==DWZ.statusCode.redirect){
alertMsg.correct(json.message||DWZ.msg("sessionTimout"),{okCall:function(){
...
loadUrl:function(url,data,callback,beforeSend,complete){
var $beforeSend = beforeSend || '';
var $complete = complete || '';
$(this).ajaxUrl({url:url,data:data,callback:callback,beforeSend:$beforeSend,complete:$complete});},
initUI:function(){
...
var iH=iRefH-iLayoutH>30?iRefH-iLayoutH:30;
if($this.isTag("table")){
...
return this.optional(element)||/^[0-9][0-9\.]*$/i.test(value);},"Numbers or dot only please");
...
return this.optional(element)||/^[a-z0-9][a-z0-9\.@_\-]*$/i.test(value);},"Letters, numbers or underscores or dot or at or plus only please");
...
numericdot:{numericdot:true},
alphanumeric:{alphanumeric:true},
alphanumericdot:{alphanumericdot:true},
...
if(node.hasClass('defaultSelected')) $('>div', node).addClass('selected');
...
return false;}
eval($this.attr("callbackNavTabBefore")||";");
navTab.openTab(tabid,url,{title:title,fresh:fresh,external:external});
eval($this.attr("callbackNavTabAfter")||";");
...
$(this).click(function(event){
var $this=$(this);
var $doBefore=$this.attr('doBefore')||'';
if(!$.isFunction($doBefore))$processUrl=eval($doBefore);
$doBefore;
var $doAfter=$this.attr('doAfter')||'';
var $attach_url=$('#attach_url_parameters').val()||'';
if($doAfter != '' && !$.isFunction($doAfter))$doAfter=eval($doAfter);
var rel=$this.attr("rel");
if(rel){
var $rel=$("#"+rel);
var url=$this.attr("href");
var separator=url.indexOf('?')<0?'?':'&';
url=$attach_url==''?url:(url+separator+$attach_url);
$rel.loadUrl(url,{},function(){
$rel.find("[layoutH]").layoutH();},'',$doAfter);}
event.preventDefault();});});
...
if(this._autoClose)this._closeTimer=setTimeout(function(){alertMsg.close()},3500);}else{
$(this._bgId).show();}
...
if(typeof options!='undefined'&&typeof options.autoClose!='undefined')this._autoClose=options.autoClose;
...
var dbClickTarget = $(this).attr('dbClickTarget');
if (dbClickTarget == 'navTab'){
navTab.openTab($(this).attr('dbClickRel'),$(this).attr('dbClickHref'),{title:$(this).attr('dbClickTitle'),fresh:false, data:{}});
}});
$tr.click(function(){
$trs.filter(".selected").removeClass("selected");
$tr.addClass("selected");
...
$.each(navTabIds,function(index,navTabId){
...
var $pagerForm=$("#pagerForm",navTab.getCurrentPanel());
var args=$pagerForm.size()>0?$pagerForm.serializeArray():{}
navTabPageBreak(args,json.rel);}else{navTab.reload('',{navTabId:navTab._getTabs().eq(navTab._currentIndex).attr('tabid')});}
if("closeCurrent"==json.callbackType){
...
if(json.navTabId){
navTab.reload(json.forwardUrl,{navTabId:json.navTabId});}else if(json.rel){
var $parentContainer=json.container?$('#'+json.container):navTab.getCurrentPanel();
var $pagerForm=$("#pagerForm",$parentContainer);
var args=$pagerForm.size()>0?$pagerForm.serializeArray():{}
navTabPageBreak(args,json.rel);}
if("closeCurrent"==json.callbackType){
$.pdialog.closeCurrent();}}}
function closeOpenNavTabAjaxDone(json){
DWZ.ajaxDone(json);
if (json.statusCode == DWZ.statusCode.ok){
if (json.navTabId){
...
currNavTabId=navTab._getTabs().filter('.selected').attr('tabid');
navTab.closeCurrentTab();
navTab.openTab(currNavTabId, json.forwardUrl, {title:json.title})
}}}
...
console.log('-------parent---------');
console.log($parent);
console.log('op.rel='+op.rel);*/
if(op.rel){
//console.log($parent.html());
var $box=$parent.find("#"+op.rel);
var form=_getPagerForm($box,op.data);
/*
console.log('-------IN: op.rel---------');
alert($box.html());
console.log('-------form---------');
console.log(form);
*/
...
// console.log(args);console.log(rel);
...
function ajaxTodo(url,callback,doBefore,obj_event){
var $doBefore='';
if (typeof doBefore!='undefined'){
if(!$.isFunction($doBefore))$doBefore=eval('('+doBefore+')');
}
...
okCall:function(){
ajaxTodo(url,$this.attr("callback"),$this.attr("doBefore"),$this);}});}else{
ajaxTodo(url,$this.attr("callback"),$this.attr("doBefore"),$this);}
event.preventDefault();});});},
dwzExport:function(){
...
patternDate:$th.attr("format")||"yyyy-MM-dd HH:mm:ss",
name:$th.attr("name")||"",
...
fieldComboxOptions:$th.attr("fieldComboxOptions")||""};
fields.push(field);});
...
break;
case 'date':
html='<input type="text" name="'+field.name+'" value="'+field.defaultVal+'" class="date '+field.fieldClass+'" dateFmt="'+field.patternDate+'" size="'+field.size+'" readonly/>'+'<a class="inputDateButton" href="javascript:void(0)">选择</a>';
break;
case 'combox':
html='<select class="combox'+field.fieldClass+'" name="'+field.name+'"><option value="-1" selected>请选择</option>'
var fieldComboxOptions=DWZ.jsonEval(field.fieldComboxOptions);
$.each(fieldComboxOptions,function(index,item){
...
var callback_select=$input.attr("callback") || ';';
...
/*
$.ajax({
type:'GET',dataType:"json",url:refUrl.replace("{value}",encodeURIComponent($this.attr("value"))),cache:false,
data:{},
...
error:DWZ.ajaxError});}
*/
var urlAjax=refUrl.replace("{value}",encodeURIComponent($this.attr("value")));
var concator=urlAjax.indexOf('?')>=0?'&':'?';
$.getJSON(urlAjax+concator+'jsoncallback=?',function(json){
if(!json)return;
var html='';
$.each(json,function(i){
if(json[i]&&json[i].length>1){
